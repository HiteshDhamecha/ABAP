...
import { LoadGeneratedEntities, AbstractEntity,  } from 'mj_generatedentities';
...
export class AppComponent implements OnInit {
  ...
  private async SetupApp(token: string, user: User): Promise<void> {
    try {
      ...
      if (err instanceof Error && 'response' in err && (err as any).response.errors) {
        const unauthorizedError = (err as any).response.errors.find((error: any) => error.extensions.code === 'UNAUTHORIZED');
